...
/* 复位状态字中C3、C2、C1和C0条件位 */
#define clear_Cx() (I387.swd &= ~0x4500)

/*
 * 对临时实数a进行规格化处理。几表示成指数、有效数形式
 * 例如：102.345表示成1.02345X10^2。0.0001234表示成1.234X10^-4。当然，函数中使用二进制表示这些数值
 */
static void normalize(temp_real * a)
{
	...
	/* 如果a的尾数最左端有0值比特位，那么将尾数左移，同时调整指数值（递减）。直到尾数的b字段最高有效位MSB是1位置（此时b表现为负值）。最后再添加上符号位 */
	while (i && a->b >= 0) {
		i--;
...
/*
 * 仿真浮点指令FTST
 * 即栈顶累加器ST(0)与0比较，并根据比较结果设置条件位。若ST>0.0，则C3，C2，C0分别为000；若ST<0.0，则条件位为001；若ST==0.0，则条件位是100；若不可比较，则条件位为111
 */
void ftst(const temp_real * a)
{
	temp_real b;

	/* 首先清状态字中条件标志位，并对比较值b(ST)进行格式化处理。若b不等于零并且设置了符号位（是负数），则设置条件位C0，否则设置条件位C3 */
	clear_Cx();
	b = *a;
	normalize(&b);
...
/* 
 * 仿真浮点指令FCOM
 * 比较两个参数src1、src2。并根据比较结果设置条件位。若src1>src2，则C3，C2，C0分别为000；若src1<src2，则条件位为001；若两者相等，则条件位是100
 */
