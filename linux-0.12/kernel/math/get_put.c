 ...
 */
/* 本程序处理所有对用户内存的访问：获取和存入指令/实数值/BCD数值等。这是涉及临时实数以外其他格式仅有的部分。所有其他运算全都使用临时实数格式 */
...
/*
 * 取用户内存中的短实数（单精度实数）。
 * 根据浮点指令代码中寻址模式字节中的内容和info结构中当前寄存器中的内容，取得短实数所在处的有效地址（math/ea.c），然后从用户数据区读取相应实数值。最后
 * 把用户短实数转换成临时实数（math/convert.c）。
...
/*
 * 取用户内存中的长实数（双精度实数）
 * 该函数处理方式与上面get_short_real()函数基本相同
 */
...
/*
 * 取用户内存中的临时实数
 * 首先根据浮点指令代码中寻址模式字节中的内容和info结构中当前寄存器中的内容，取得临时实数所在处的有效地址，然后从用户数据区读取相应临时实数值
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
 ...
 * 该函数首先根据浮点指令代码中寻址模式字节中的内容和info结构中当前寄存器中的内容，取得短整数所在有效地址，然后从用户数据区读取相应整数值，并保存为临时整数格式。最后把临时
 * 整数值转换成临时实数。
 * 临时整数也用10字节表示。其中低8字节是无符号整数值，高2字节表示指数值和符号位。如果高2字节最高有效位为1，则表示是负数；若最高有效位是0，表示是正数
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
...
/*
 * 去用户内存中的64位长整数并转换成临时实数格式。
 * 首先根据浮点指令代码中寻址模式字节中的内容和info结构中当前寄存器中的内容，取得64位长整数所在有效地址，然后从用户数据区读取相应整数值，并保存为临时整数格式。最后
 * 再把临时整数值转换成临时实数。
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
	...
	struct info * info, unsigned short code)
{
	...
	if ((ti.sign = (ti.b < 0)))									/* 若是负数，则设置临时整数符号位 */
...
/* 64位加法。把32位的无符号数val加到64位数<high,low>中 */
 ...
 * 该函数首先根据浮点指令代码中寻址模式字节中的内容和info结构中当前寄存器中的内容，取得BCD码所在有效地址（math/ea.c），然后从用户数据区读取10字节相应BCD码值（其中1字节用于符号），
 * 同时转换成临时整数形式。最后把临时整数值转换成临时实数。
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
void get_BCD(temp_real * tmp, struct info * info, unsigned short code)
{
	...
	/*
	 * 取得BCD码数值所在内存有效地址。然后从最后1个BCD码字节（最高有效位）开始处理。先取得BCD码数值的符号位，并设置临时整数的符号位。然后把9字节的BCD码值转换成临时整数格式，
	 * 最后再把临时整数值转换成临时实数。
	 */
 ...
 * 该函数首先根据浮点指令代码中寻址模式字节中的内容和info结构中当前寄存器中的内容，取得保存结果的有效地址addr，然后把临时实数格式的结果转换成短实数格式并存储到有效地址addr处
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
 ...
 * 该函数处理方式与上面put_short_real()函数基本相同
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
 ...
 * 该函数首先取得保存结果用的地址addr，然后把临时实数存储到有效地址addr处
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
 ...
 * 该函数首先取得保存结果用的地址addr，然后把临时实数格式的结果转换成临时整数格式。如果是负数则设置整数符号位。最后把整数保存到用户内存中
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
	...
	struct info * info, unsigned short code)
{
	...
	if (ti.sign)												/* 若有符号位，则取负数值 */
		ti.a = -ti.a;
	put_fs_word(ti.a,(short *) addr);							/* 存储数据到用户数据区 */
}
	...
	struct info * info, unsigned short code)
{
	...
	if (ti.sign)												/* 若有符号位，则取负数值 */
		ti.a = -ti.a;
	put_fs_long(ti.a,(unsigned long *) addr);					/* 存储数据到用户数据区 */
}
 ...
 * 该函数首先取得保存结果用的地址addr，并验证保存10字节BCD码的用户空间。然后把临时实数格式的结果转换成BCD码格式的数据并保存到用户内存中。如果是负数则设置最高存储字节的最高有效位。
 * 参数：tmp - 转换成临时实数后的指针；info - info结构指针；code - 指令代码
 */
