...
/*
 * 临时实数加法子程序
 * 注意！这些并不精确：它们只有62比特宽度，并且不能进行正确地舍入操作。这些仅是草就之作。
 * 原因是为了不会溢出（1比特），我们把值右移了2位，并且使得符号位（1比特）能够移入尾数中。
 * 这是非常简单的算法，而且62位（实际上是61位-没有舍入）的精度通常也足够了。只是当你把64位
 * 的整数相加时才会发觉一些奇怪的问题。当使用双精度（52比特精度）数据时，是永远不可能超过
 * 61比特精度的
 */
...
/*
 * 尾数符号化
 * 即把临时实数变换成指数和整数表示形式，以便于仿真运算。因此我们这里称其为仿真格式。具体操作是把64位二进制位数右移2位（因此指数需要加2）。因为指数字段exponent的最高比特位是符号位
 * 所以若指数值小于零，说明该数是负数。于是则把位数用补码表示（取负）。然后把指数取正值。此时尾数中不仅包含移过2位的有效数，而且包含数值的符号位。
 */
static void signify(temp_real * a)
{
	/* 下面嵌入汇编代码：%0是a->a；%1是a->b。汇编指令"shrdl $2, %1, %0"执行双精度（64位）右移，即把组合位数<b,a>右移2位。由于该移动操作不会改变%1（a->b）中的值，因此还需要单独对其右移2位 */
	a->exponent += 2;										/* 指数值增2 */
...
{
	/*
	 * 对于值为0的数不用处理，直接返回。否则，我们先复制临时实数格式的符号位，然后判断尾数的高位long字段a->b是否带有符号位。若有，则在exponent字段添加符号位，同时
	 * 把尾数用无符号数形式表示（取补）出来。最后对尾数进行规格化处理，同时指数值作相应递减。即执行左移操作，使得尾数最高有效位不为0（最后a->b值表现为负值）。
	 */
	if (!(a->a || a->b)) {									/* 若值为0就返回 */
		a->exponent = 0;
		return;
...
void fadd(const temp_real * src1, const temp_real * src2, temp_real * result)
{
	...
	/* 首先取两个数的指数值x1、x2（去掉符号位）。然后让变量a等于其中最大值，shift为指数差值（即相差2的倍数值）。 */
	...
	/* 
	 * 若两者相差太大，大于等于2的64次方，则我们可以忽略小的那个数，即b值。于是直接返回a值即可。否则，若相差大于等于2的32次方，那么我们可以忽略小值b中的低32位值。于是我们
	 * 把b的高long字段值b.b右移32位，即放到b.a中。然后把b的指数值相应地增加32次方。即指数差值减去32.这样调整之后，相加的两个数的尾数基本上落在相同区域中。
	 */
	if (shift >= 64) {
		*result = a;
		return;
	...
	/*
	 * 接着再进行细致地调整，以将相加两者的指数调整成相同值。调整方法是把小值b的尾数右移shift个比特位。这样两者的指数相同，处于同一个数量级下。我们就可以对尾数进行相加运算了。
	 * 相加之前我们需要先把它们转换成仿真运算格式，并在加法运算后再变回临时实数格式
	 */
