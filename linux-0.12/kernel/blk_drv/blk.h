 ...
 */
/*
 * 下面定义的NR_REQUEST是请求队列中所包含的项数。注意，写操作仅使用这些项中低端的2/3项；读操作优先处理。
 * 32项好像是一个合理的数字；该数已经足够从电梯算法中获得好处，但当缓冲区在队列中而锁住时又显得是很大的数
 ...
 */
/*
 * ok，下面是request结构的一个扩展形式，因而当实现以后，我们就可以在分页请求中使用同样的request结构。
 * 在分页处理中，'bh'是NULL，而'waiting'则用于等待读/写的完成
 */
/*
 * 下面是请求队列中请求项的结构。其中如果字段dev=-1，则表示队列中该项没有被使用。字段cmd可取常量READ(0)或WRITE(1)（定义在include/linux/fs.h中）
 * 其中，内核并没有用到waiting指针，取而代之地内核使用了缓冲块的等待队列。因为等待一个缓存块与等待请求项完成是对等的
 */
struct request {
 ...
 */
/* 下面的定义用于电梯算法：注意读操作总是在写操作之前进行。这是很自然的；读操作对时间的要求比写操作严格得多 */
/*
 * 下面宏中参数s1和s2的取值是上面定义的请求结构request的指针。该宏定义用于根据s1和s2指定的请求项结构中的信息（命令cmd（READ或WRITE）、设备号dev以及所操作
 * 的扇区号sector）来判断出两个请求项结构的前后排列顺序。这个顺序将用作访问块设备时的请求项指向顺序。这个宏会在函数add_request()中被使用（blk_drv/ll_rw_blk.c）
...
/* 块设备表（数组）。每种块设备占用一项，共7项。表索引值即是主设备号 */
...
/*
 * 一个块设备上数据块总数指针数组。每个指针项指向指定主设备号的总块数数组hd_sizes[]（blk_drv/hd.c）。该总块数数组每一项对应一个子设备上
 * 所拥有的数据块总数（1块大小=1KB）
 */
extern int * blk_size[NR_BLK_DEV];

/* 在包含此头文件的块设备驱动程序（如hd.c）中，必须先定义程序使用的主设备号。这样，下面就能为包含本文件的驱动程序给出正确的宏定义 */
#ifdef MAJOR_NR
...
#define DEVICE_NR(device) (MINOR(device)/5)	/* 子设备号（0-1） */
#define DEVICE_ON(device)					/* 一开机硬盘就总是运转着 */
#define DEVICE_OFF(device)					/* 一开机硬盘就总是运转着 */
...
/* 为了便于编程，这里定义了两个宏：CURRENT是指定设备号的当前请求结构项指针，CURRENT_DEV是当前请求项CURRENT中设备号 */
...
/*
 * 解锁指定的缓冲块
 * 如果指定缓冲块bh并没有被上锁，则显示警告信息。否则将该缓冲块解锁，并唤醒等待该缓冲块的进程。此为内嵌函数，但用做”宏“。参数是缓冲块头指针
 */
 ...
 * 结束请求处理”宏“
 * 参数uptodate是更新标志
 * 首先关闭指定块设备，并解锁该缓冲区。如果更新标志参数值是0，表示此次请求项的操作已失败，因此显示相关块设备IO错误信息。最后，唤醒等待该请求项的进程
 * 以及等待空闲请求项出现的进程，释放并从请求链表中删除本请求项，并把当前请求项指针指向下一请求项
 */
...
/*
 * 定义初始化请求项宏
 * 由于几个块设备驱动程序开始处对请求项的初始化操作相似，因此这里为它们定义了一个统一的初始化宏。该宏用于对当前请求项进行一些有效性判断。所做工作如下：
 * 如果设备当前请求项为空（NULL），表示本设备目前已无需要处理的请求项。于是略作扫尾工作就退出相应函数。否则，如果当前请求项中设备的主设备号不等于驱动程序
 * 定义的主设备号，说明请求项队列乱掉了，于是内核显示出错信息并停机。否则若请求项中用的缓冲块没有被锁定，也说明内核程序出了问题，于是也显示出错信息并停机
 */
#define INIT_REQUEST \
repeat: \
	if (!CURRENT) {\
