 ...
 */
/*
 * 该函数在整个内核中使用（包括在头文件*.h，内存管理程序mm和文件系统fs中），
 * 用以指出主要的出错问题
 */
 ...
 * 函数名前的关键字volatile用于告诉编译器gcc该函数不会返回。这样可以让gcc产生更好一些的代码，更重要的是使用这个关键字可以避免产生某些（未初始化变量的）
 * 假警告信息。等同于现在gcc的函数属性说明：void panic(const char *s) __attribute__ ((noreturn))
 */
