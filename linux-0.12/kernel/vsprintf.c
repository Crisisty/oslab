 ...
 */
/*
 * Lars Wirzenius是Linus的好友，在Helsinki大学时曾同处一间办公室。在1991年夏季开发Linux时，Linus当时对C语言还不是很熟悉，还不会使用可变参数
 * 列表函数功能。因此Lars Wirzenius就为他编写了这段用于内核显示信息的代码。他后来（1998年）承认在这段代码中有一个bug，直到1994年才有人发现，并予以
 * 纠正。这个bug是在使用*作为输出域宽度时，忘记递增指针跳过这个星号了。在本代码中这个bug还仍然存在。
 */

#include <stdarg.h>		/* 标准参数头文件。以宏的形式定义变量参数列表。主要说明一个类型（va_list）和3个宏（va_start，va_arg和va_end）
	...
	,int type)
{
	...
	/* 
	 * 如果类型type指出用小写字母，则定义小写字母集。如果类型指出要左调整（靠左边界），则屏蔽类型中的填零标志。
	...
	/* 
	 * 如果类型指出要填零，则置字符变量c='0'，否则c等于空格字符。如果类型指出是带符号数并且数值num小于0，则置符号变量sign=负号，并使num取绝对值
	...
	/*
	 * 若带符号，则宽度值减1。若类型指出是特殊转换，则对于十六进制宽度再减少2位（用于0x），对于八进制宽度减1（用于八进制转换结果前放一个零）
	 */
	if (sign){
		size--;
	}
	...
	/*
	 * 如果数值num为0，则临时字符串='0'；否则根据根据给定的基数将数值num转换成字符形式。若数值个数大于精度数，则精度数扩展为数字个数。
	 * 宽度size减去用于存放数值字符的个数
	 */
	i = 0;
	if (num == 0) {
	...
	size -= precision;
	/*
	 * 从这里才开始逐步生成转换结构，并暂时放在字符串str中。若类型中没有填零（ZEROPAD）和左靠齐（左调整）标志，则在str中首先填放剩余宽度值指出的空格数。
	 * 若需带符号位，则存入符号
	 */
	...
	/* 此时i存有数值num的数字个数。若数字个数小于精度值，则str中放入（精度值-i）个'0'。然后再将数值转换好的数字字符填入str中。共i个 */
	while(i<precision--) {
		*str++ = '0';
	}
	while(i-->0) {
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
	...
	/*
	 * 首先将字符指针指向buf，然后扫描格式字符串，对各个格式转换指示进行相应的处理。格式转换指示字符串均以'%'开始。这里从fmt格式字符串中扫描'%'
	...
	for (str=buf ; *fmt ; ++fmt) {
		...
		/* 下面取得格式指示字符串的标志域，并将标志常量放入flags变量中 */
		/* process flags */
		flags = 0;
		repeat:
		...
		/*
		 * 取当前参数字段宽度域值放入field_width变量中。如果宽度域中是数值则直接取其为宽度值。如果宽度域中是字符'*'，表示下一个参数指定宽度。
		 * 因此调用va_arg取宽度值。若此时宽度值小于0，则该负数表示其带有标志域'-'标志（左靠齐），因此还需在标志变量中添入该标志，并将字段
		 * 宽度值取为其绝对值
		 */
		/* get field width */
		field_width = -1;
		...
		/*
		 * 下面这段代码取格式转换串的精度域，并放入precision变量中。精度域开始的标志是'.'。其处理过程与上面宽度域的类似。如果精度域中是数值则直接取其
		 * 为精度值。如果精度域中是字符'*'，表示下一个参数指定精度。因此调用va_arg取精度值。若此时宽度小于0，则将字段精度值取为0
		 */
		/* get the precision */
		precision = -1;
		if (*fmt == '.') {
		...
		/* 下面这段代码分析长度修饰符，并将其存入qualifier变量 */
		...
		switch (*fmt) {
		/*
		 * 如果转换指示符是'c'，则表示对应参数应是字符。此时如果标志域表明不是左靠齐，则该字段前面放入'宽度域值-1'个空格字符，然后再放入参数字符。
		 * 如果宽度域还大于0，则表示为左靠齐，则在参数字符后面添加'宽度域值-1'个空格字符
		 */
		case 'c':
			if (!(flags & LEFT))
				while (--field_width > 0)
		...
		/*
		 * 如果转换指示符是's'，则表示对应参数是字符串。首先取参数字符串的长度，若其超过精度域值，则扩展精度域=字符串长度。此时如果标志域表明不是
		 * 左靠齐，则该字段前放入'宽度值-字符串长度'个空格字符。然后再放入参数字符串。如果宽度域还大于0，则表示为左靠齐，则在参数字符串后面添加
		 * '宽度值-字符串长度'个空格字符
		 */
		case 's':
			s = va_arg(args, char *);
		...
		/* 
		 * 若格式转换指示符是'n'，则表示要把到目前为止转换输出字符数保存到对应参数指针指定的位置中。首先利用va_arg()取得该参数指针，
		...
		/*
		 * 若个数转换符不是'%'，则直接将一个'%'写入输出串中。如果格式转换符的位置处还有字符，则也直接将该字符写入输出串中，并循环继续处理格式字符串。
		 * 否则表示已处理到格式字符串结尾处
		 */
		default:
			if (*fmt != '%') {
