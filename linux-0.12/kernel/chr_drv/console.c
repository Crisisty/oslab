...
/*
 * console.c
 * 该模块实现控制台输入输出功能 'void con_init(void)' 'void con_write(struct tty_queue *queue)' 希望这是一个非常完整的VT102实现。
 * 感谢John T Kohl 实现了蜂鸣指示子程序
 * 虚拟控制台、屏幕黑屏处理、屏幕拷贝、彩色处理、图形字符显示以及VT100终端增强操作有Peter MacDonald编制 
 */
 ...
 */
/*
 * 注意！！！我们有时短暂地禁止和允许中断（当输出一个字（word）到视频IO），但即使对于键盘中断这也是可以工作的。因为我们使用陷阱门，所以我们知道在处理
 * 一个键盘中断过程期间中断时被禁止的。希望一切均正常
 */
...
/* 该符号常量定义终端IO结构的默认数据（include/termios.h） */
#define DEF_TERMIOS 		                                \
	(struct termios) { 		                                \
	        ICRNL, 			                                \
        	OPOST | ONLCR,                                  \
...
/* 参加对boot/setup.s的注释和setup程序读取并保留的系统参数表 */
...
/*
 * 定义虚拟控制台结构，它包含一个虚拟控制台的当前所有信息。其中vc_origin和vc_src_end是当前正在处理的虚拟控制台执行快速滚屏操作时使用的起始行和末行对应的显示内存位置。
 * vc_video_mem_start和vc_video_mem_end是当前虚拟控制台使用的显示内存区域部分。vc-Virtual Console
 */
static struct {
...
/* 为了便于引用，以下定义当前正在处理控制台信息的符号。含义同上。其中currcons是使用vc_cons[]结构的函数参数中的当前虚拟终端号 */
 ...
 */
/* 下面是终端辉英ESC-Z或csi0c请求的应答（=vt100响应） */
/*
 * csi-控制序列引导码（Control Sequence Introducer）
 * 主机通过发送不带参数或参数是0的设备属性（DA）控制序列（'ESC [c'或'ESC [0c'）要求终端应答一个设备属性控制序列（ESC Z的作用与此相同），终端则发送以下
 * 序列来响应主机。该序列（即'ESC [?1;2c'）表示终端是具有高级视频功能的VT100兼容终端
 */
#define RESPONSE "\033[?1;2c"

/* 定义使用的字符集。其中上半部分是普通7比特ASCII代码，即US字符集。下半部分对应VT100终端设备中的线条字符，即显示图表线条的字符集 */
 ...
 * 该函数用于更新当前光标位置变量x，y，并修正光标在显示内存中的对应位置pos。该函数会首先检查参数的有效性。如果给定的光标列号超出显示器最大列数，或者光标行号不低于显示的最大行数。
 * 则退出。否则就更新当前光标变量和新光标位置对应在现实内存中位置pos。
...
/*
 * 设置滚屏起始显示内存地址
 * 首先检查显示卡类型是否彩色卡，并判断参数指定的控制台是否前台控制台，如果不满足这两条件则退出。否则就向显示卡上寄存器输出滚屏起始地址
 */
static inline void set_origin(int currcons)
{
	/*
	 * 首先判断显示卡类型。对于EGA/VGA卡，我们可以指定屏内行范围（区域）进行滚屏操作，而MDA单色显示卡只能进行整屏滚屏操作。因此只有EGA/VGA卡才需要设置滚屏起始行显示内存地址
	 * （起始行是origin对应的行），即显示类型如果不是EGA/VGA彩色模式，也不是EGA/VGA单设模式，那么就直接返回。另外，我们只对前台控制台进行操作，因此只有当前控制台currcons是
	 * 前台控制台时，我们才需要设置滚屏起始行对应的内存起点位置
	 */
	...
	if (currcons != fg_console)
		return;
	/*
	 * 然后向显示寄存器选择端口video_port_reg输出12，即选择显示控制数据寄存器r12，接着写入滚屏其实地址高字节。其中向右移动9位，实际上表示向右移动8位再除以2（屏幕上1个字符用2字节表示）
	 * 再选择显示控制数据寄存器r13，然后写入滚屏起始地址低字节。向右移动1位表示除以2，同样代表屏幕上1个字符用2字节表示。输出值相对于默认显示内存起始位置video_mem_base进行操作，例如对于
	 * EGA/VGA彩色模式，video_mem_base=物理内存地址0xb8000
	 */
	cli();
...
/*
 * 显示内容向上卷动一下
 * 将屏幕滚动窗口向下移动一行，并在屏幕滚动区域底出现的新行上添加空格字符。滚屏区域必须大于1行
 */
static void scrup(int currcons)
{
	/*
	 * 滚屏区域必须起码有2行。如果滚屏区域顶行号大于等于区域底行号，则不满足进行滚行操作的条件。另外，对于EGA/VGA卡，我们可以指定屏内行范围（区域）进行滚屏操作，而MDA单色显示卡只能进行
	 * 整屏滚屏操作。该函数对EGA和MDA显示类型进行分别处理。如果显示类型是EGA，则还分为整屏窗口移动和区域内窗口移动。这里首先处理显示卡是EGA/VGA显示类型的情况
	 */
	if (bottom<=top)
		return;
	if (video_type == VIDEO_TYPE_EGAC || video_type == VIDEO_TYPE_EGAM)
	{
		/*
		 * 如果移动起始行top=0，移动最底行bottom=video_num_lines=25，则表示整屏窗口向下移动一行。于是把整个屏幕窗口左上角对应的起始内存位置origin调整为向下移一行对应的内存位置。
		 * 同时也跟踪调整当前光标对应的内存位置以及屏幕末行末端字符指针scr_end的位置。最后把新屏幕窗口内存起始位置值origin写入显示控制器中
		 */
		if (!top && bottom == video_num_lines) {
			...
			/*
			 * 如果屏幕窗口末端所对应的显示内存指针src_end超出了实际显示内存末端，则将屏幕内容除第一行以外所有行对应的内存数据移动到显示内存的起始位置vedio_mem_start处，并在整屏窗口
			 * 向下移动一行后出现的新行上填入空格字符。然后根据屏幕内存数据移动后的情况，重新调整当前屏幕对应内存的起始指针、光标位置指针和屏幕末端对应内存指针src_end。这段嵌入汇编程序首先
			 * 将（屏幕字符行数-1）行对应的内存数据移动到显示内存起始位置video_mem_start处，然后在随后的内存位置处添加一行空格（擦除）字符数据。
			...
			set_origin(currcons);
		/*
		 * 否则表示不是整屏移动。即表示从指定行top开始到bottom区域中的所有行向上移动1行，指定行top被删除。此时直接将屏幕从指定行top到bottom所有行对应的显示内存数据向上移动
		 * 1行，并在最下面新出现的行上填入擦除字符
	...
	/*
	 * 如果显示类型不是EGA（而是MDA），则执行下面移动操作。因为MDA显示控制卡只能整屏滚动，并且会对超出显示内存范围的情况自动调整（即会自动翻卷指针），所以这里不对屏幕
	 * 内容对应内存超出显示内存范围的情况单独处理。处理方法与EGA非整屏移动情况完全一样
	 */
	else		/* Not EGA/VGA */
	{
		__asm__("cld\n\t"
...
/*
 * 显示内容向下卷动一行
 * 将屏幕滚动窗口向上移动一行，相应屏幕滚动区域内容向下移动1行。移动开始行的上方会出现一新行。处理方法与scrup()相似，只是为了在移动显示内存数据时不会出现数据覆盖的问题
 * 复制操作时以逆向进行的，即从屏幕倒数第2行的最后一个字符开始复制到最后一行，再讲倒数第3行复制到倒数第2行等等。因此此时对EGA/VGA显示类型和MDA类型的处理过程完全一样
 * 所以该函数实际上没有必要写两段相同的代码，即这里if和else语句块中的操作完全一样
 */
static void scrdown(int currcons)
{
	/*
	 * 同样地，滚动区域必须起码有2行。如果滚动区域顶行号大于等于区域底行号，则不满足进行滚行操作的条件。另外，对于EGA/VGA卡，我们可以指定屏内行范围（区域）进行滚屏操作
	 * 而MDA单色显示卡只能进行整屏滚屏操作。由于窗口向上移动最多移动到当前控制台占用显示区域内存的起始位置，因此不会发生屏幕窗口末端所对应的显示内存指针src_end超出实际
	 * 显示内存末端的情况，所以这里只需要处理普通的内存数据移动情况。
	 */
	if (bottom <= top)
		return;
	if (video_type == VIDEO_TYPE_EGAC || video_type == VIDEO_TYPE_EGAM)
...
/*
 * 光标在同列位置下移一行
 * 如果光标没有处在最后一行上，则直接修改光标当前行变量y++，并调整光标对应显示内存位置pos（加上一行字符所对应的内存长度）。否则就需要将屏幕窗口内容上移一行
...
/*
 * 光标在同列上移一行
 * 如果光标不在屏幕第一行上，则直接修改光标当前行变量y--，并调整光标对应显示内存位置pos，减去屏幕上一行字符所对应的内存长度字节数。否则需要将屏幕窗口内容下移一行。
 ...
 * 如果光标没有处在0列，则将光标对应的内存位置pos后退2字节（对应屏幕上一个字符），然后将当前光标变量列值减1，并将光标所在位置处字符擦除
 */
...
/*
 * 删除屏幕上与光标位置相关的部分内容。
 * 本函数处理ANSI转义或控制序列。根据指定的控制序列，执行与光标位置相关的字符删除操作，擦除部分或所有显示的字符，并且在擦除字符或行时光标位置不变。
 * 该函数处理的ANSI控制序列为：'ESC [ Ps J'。其中，Ps = 0 - 表示删除光标处到屏幕底端；1 - 删除屏幕开始到光标处；2 - 整屏删除。
 * 函数名称csi_J（CSI - Control Sequence Introducer，即控制序列引导码）指明对控制序列"CSI Ps J"进行处理。参数vpar对应上面控制序列中Ps的值。
 */
static void csi_J(int currcons, int vpar)
{
	long count;					/* 设为使用寄存器变量 */
	long start;

	/* 首先根据上述三种情况分别设置需要删除的字符数和删除开始的显示内存位置。 */
	switch (vpar) {
...
/*
 * 删除一行上与光标位置相关的部分
 * 本函数根据参数擦除光标所在行的部分或所有字符。擦除操作从屏幕上移走字符但不影响其他字符。擦除的字符被丢弃。在擦除字符或行时光标位置不变。
 * ANSI转义字符序列：'ESC [ Ps K'（Ps = 0 删除到行尾；1从开始删除；2整行都删除）
 * 参数：vpar-对应控制序列中Ps
 */
static void csi_K(int currcons, int vpar)
{
	long count;									/* 设置使用寄存器变量 */
	long start;
 ...
 * ANSI转义序列：'ESC [ Ps;Ps m’。其中Ps = 0 - 默认属性；1 - 粗体并增亮；4 - 下划线；5 - 闪烁；7 - 反显；22 - 非粗体；24 - 无下划线；25 - 无闪烁；27 - 正显；
 * 30 - 38 - 设置前景色彩；39 - 默认前景色（White）；40 - 48 - 设置背景色彩；49 - 默认背景色（Black）。
 */
void csi_m(int currcons )
{
	...
	/*
	 * 一个控制序列中可以带有多个不同参数。参数存储在数组par[]中。下面就根据接收到的参数个数npar，循环处理各个参数Ps
	 * 如果Ps = 0，则把当前虚拟控制台随后显示的字符属性设置为默认属性def_attr。初始化时def_attr已被设置成0x07（黑底白字）。
	 * 如果Ps = 1，则把当前虚拟控制台随后显示的字符属性设置为粗体或增亮显示。如果是彩色显示，则把字符属性或上0x08让字符高亮显示；如果是单色显示，则让字符带下划线显示。
	 * 如果Ps = 4，则对彩色和单色显示进行不同的处理。若此时不是彩色显示方式，则让字符带下划线显示。如果是彩色显示，那么若原来vc_bold_attr不等于-1时就复位其背景色；
...
{
	/*
	 * 既然我们需要设置显示光标，说明有键盘操作，因此需要恢复进行黑屏操作的延时计数值。另外，显示光标的控制台必须是前台控制台，因此若当前处理的台号currcons不是
	 * 前台控制台就立刻返回
	 */
	...
	if (currcons != fg_console)
		return;
	/*
	 * 然后使用索引寄存器端口选择显示数据寄存器r14（光标当前显示位置高字节），接着写入光标当前位置高字节（向右移动9位表示高字节移到低字节再除以2）。这是相对于默认
...
{
	/*
	 * 首先使用索引寄存器端口选择显示控制数据寄存器r14（光标当前显示位置高字节），然后写入光标当前位置高字节（向右移动9位表示高字节移到低字节再除以2）。是相对于默认
	 * 显示内存操作的。再使用索引寄存器选择r15，并将光标当前位置低字节写入其中
	 */
	outb_p(14, video_port_reg);
...
/*
 * 发送对VT100的响应序列
 * 即为响应主机请求终端向主机发送设备属性（DA）。主机通过发送不带参数或参数是0的DA控制序列（'ESC [ 0c'或'ESC Z'）要求终端发送回一个设备属性（DA）控制序列。终端则
 * 发送回'ESC [?1;2c'来响应主机的序列。该序列告诉主机本终端是具有高级视频功能的VT100兼容终端。处理过程是将应答序列放入读缓冲队列中，并使用函数copy_to_cooked()
 * 处理后放入辅助队列中
 */
...
static void insert_line(int currcons)
{
	...
	/* 首先保存屏幕窗口卷动开始行top和最后行bottom值，然后从光标所在行让屏幕内容向下滚动一行。最后恢复屏幕窗口卷动开始行top和最后行bottom的原来值 */
	oldtop=top;
	oldbottom=bottom;
	top=y;										/* 设置屏幕卷动开始行和结束行 */
...
static void delete_char(int currcons)
{
	...
	/* 如果光标当前列位置x超出屏幕最右列，则返回。否则从光标右一个字符开始到行末所有字符左移一格，然后在最后一个字符处填入擦除字符 */
...
static void delete_line(int currcons)
{
	...
	/* 首先保存屏幕卷动开始行top和最后行bottom值，然后从光标所在行让屏幕内容向上滚动一行。最后恢复屏幕卷动开始行top和最后行bottom的原来值 */
	oldtop=top;
	oldbottom=bottom;
	top=y;										/* 设置屏幕卷动开始行和最后行 */
	bottom = video_num_lines;
...
/*
 * 在光标处插入nr个字符
 * 该函数处理ANSI转义字符序列：'ESC [ Pn @'，在当前光标处插入1个或多个空格字符。Pn是插入字符数，默认是1.光标将仍然处于第1个插入的空格字符处。在光标
 * 与右边界的字符将右移，超过右边界的字符将被丢失。参数nr = 转义字符序列中的参数Pn
 */
static void csi_at(int currcons, unsigned int nr)
{
	/* 如果插入的字符数大于一行字符数，则截取一行字符数；若插入字符数nr为0，则插入1个字符。然后循环插入指定个空格字符。 */
...
/* 
 * 在光标位置处插入nr行
 * 处理ANSI转义字符序列：'ESC [ Pn L'，该控制序列在光标处插入1行或多个空行。操作完成后光标位置不变。当空行被插入时，光标以下滚动区域内的行向下移动。
 * 滚动出显示页的行就丢失。参数nr = 转义字符序列中的参数Pn
 */
static void csi_L(int currcons, unsigned int nr)
{
	/* 如果插入的行数大于屏幕最多行数，则截为屏幕显示行数；若插入行数nr为0，则插入1行。然后循环输入指定行数nr的空行 */
...
/*
 * 删除光标处的nr个字符。
 * 处理ANSI转义字符序列：'ESC [ Pn P'，该控制序列从光标处删除Pn个字符。当一个字符被删除时，光标右所有字符都左移。这会在右边界处产生一个空字符。其属性应该与最后一个
 * 左移字符相同，但这里作了简化处理，仅使用字符的默认属性（黑底白字空格0x0720）来设置空字符。参数nr = 转义字符序列中的参数Pn
 */
...
/* 
 * 删除光标处nr行
 * 处理ANSI转义序列：'ESC [ Pn M'，该控制序列在滚动区域内，从光标所在行开始删除1行或多行。当行被删除时，滚动区域内的被删除行以下的行会向上移动，并且会在最底行添加1空行。
 * 若Pn大于显示页上剩余行数，则本序列仅删除这些剩余行，并对滚动区域外不起作用。参数nr = 转义字符序列中的参数Pn
 */
static void csi_M(int currcons, unsigned int nr)
{
	/* 如果删除的行数大于屏幕最多行数，则截为屏幕显示行数；若欲删除的行数nr为0，则删除1行。然后循环删除指定行数nr */
	if (nr > video_num_lines)
		nr = video_num_lines;
	else if (!nr)
		nr=1;
...
/* 
 * 这个枚举定义用于在下面con_write()函数中解析转义字符序列或控制序列。ESnormal是初始进入状态，也是转义或控制序列处理完毕时的状态
 * ESnormal   - 表示处于初始正常状态。此时若接收到的是普通显示字符，则把字符直接显示在屏幕上；若接收到的是控制字符（例如回车字符），则对光标位置进行设置。当刚处理
 * 			    完一个转义或控制序列，程序也会返回到本状态
 * ESesc      - 表示接收到转义序列引导字符ESC（0x1b = 033 = 27）；如果在此状态下接收到一个'['字符，则说明是转义序引导码，于是跳转到ESsquare去处理。否则就把接收到的字符
 * 			    作为转义序列来处理。对于选择字符集转义序列'ESC ('和'ESC )'，我们使用单独的状态ESsetgraph来处理；对于设备控制字符串序列'ESC P'，我们使用单独的状态ESsetterm来处理
 * ESsquare   -  表示已经接收到一个控制序列引导码（'ESC ['），表示接收到的是一个控制序列。于是本状态对参数数组par[]执行清零初始化工作。如果此时接收到的又是一个'['字符，则表示收到了'ESC [['
 * 			   序列。该序列是键盘功能键发出的序列，于是跳转到Esfunckey去处理。否则我们需要准备接收控制序列的参数，于是置状态Esgetpars并直接进入该状态去接收并保存序列的参数字符
 * ESgetpars  - 该状态表示我们此时要接收控制序列的参数值。参数用十进制数表示，我们把接收到的数字字符转换成数值并保存到par[]数组中。如果收到一个分号';'，则还是维持在本状态，并把接收到的参数值
 * 			    保存在数据par[]下一项中。若不是数字字符或分号，说明已取得所有参数，那么就转移到状态ESgotars去处理
 * ESgotpars  - 表示我们已经接收到一个完整的控制序列。此时我们可以根据本状态接收到的结尾字符对相应控制序列进行处理。不过在处理之前，如果我们在ESsquare状态收到过'?'，说明这个序列是终端设备私有的
 *   		    序列。本内核不支持对这种序列的处理，于是我们直接恢复到ESnormal状态。否则就去执行相应控制序列，待序列处理完后就把状态恢复到ESnormal
 * ESfunckey  - 表示我们接收到了键盘上功能键发出的一个序列，我们不用显示。于是恢复到正常状态ESnormal
 * ESsetterm  - 表示处于设备控制字符串序列状态（DCS）。此时若收到字符'S'，则恢复初始的显示字符属性。若收到的字符是'L'或'l'，则开启或关闭折行显示方式
 * ESsetgraph - 表示收到设置字符集转义序列'ESC ('或'ESC )'。它们分别用于指定G0和G1所用的字符集。此时若收到字符'0'，则选择图形字符集作为G0和G1，若收到的字符是'B'，这选择普通ASCII字符集作为G0和G1的字符集
 */
...
/*
 * 控制台写函数
 * 从终端的tty写缓冲队列中取字符，并针对每个字符进行分析。若是控制字符或转义或控制序列，则进行光标定位、字符删除等控制处理；对于普通字符就直接在光标处显示
...
void con_write(struct tty_struct * tty)
{
	...
	/*
	 * 该函数首先根据当前控制台使用的tty在tty表中的项位置取得对应的控制台号currcons，然后计算出（CHARS()）目前tty写队列中含有的字符数nr，并循环取出其中的每个字符进行处理。
	 * 不过如果当前控制台由于接收到键盘或程序发出的暂停命令（如按键Ctrl-S）而处于停止状态，那么本函数就停止处理写队列中的字符，退出函数。另外，如果取出的是控制夫妇CAN（Cancel，ASCII码24，
	 * 由按键Ctrl-X产生）或SUB（Substitute，26，Ctrl-Z），那么若字符是在转义或控制序列期间收到的，则序列不会执行而会立刻终止，同时显示随后的字符。注意，con_write()函数只处理在取队列
	 * 字符数时，写队列中当前含有的字符。这有可能在一个序列被放到写队列期间读取字符数，因此本函数前一次退出时state有可能正处于处理转义或控制序列的其他状态上
	 */
	...
	while (nr--) {
		...
		switch(state) {
			/*
			 * 如果取出的字符是普通显示字符代码，就直接从当前映射字符集中取出对应的显示字符，并放到当前光标所处的显示内存位置处，即直接显示该字符。然后把光标位置右移一个字符位置。
			 * 具体来讲，如果字符不是控制字符也不是扩展字符，即（31<c<127），那么，若当前光标处在行末端货末端以外位置，则将光标移到下一行头列。并调整光标位置对应的内存指针pos。
			 * 然后将字符c写到现实内存pos处，并将光标右移1列，同时相应地也将pos移动2个字节
			 */
			case ESnormal:
				 ...
				 * 如果c是DEL(127)，则将光标左边字符擦除（用空格字符替代），并将光标移动到被擦除位置
				 ...
				 * 如果字符c是控制字符S0(14)或SI(15)，则相应选择字符集G1或G0作为显示字符集
				 */
				} else if (c==9) {						/* HT - 水平制表 */
					c=8-(x&7);
					x += c;
					...
					sysbeep();
			  	else if (c == 14)						/* S0 - 换出，使用G1 */
			  		translate = GRAF_TRANS;
			  	else if (c == 15)						/* SI - 换进，使用G0 */
					translate = NORM_TRANS;
				break;
			/* 如果在ESnormal状态下收到转义字符ESC（0x1b = 033 = 27），则转到本状态处理。该状态对C1控制字符或转义字符进行处理。处理完后默认的状态将是ESnormal */
			case ESesc:
				state = ESnormal;
				switch (c)
				{
				...
				break;
			/*
			 * 如果在状态ESesc（是转义字符ESC）时收到字符'['，则表明是CSI控制序列，于是转到状态ESsequare来处理。首先对ESC转义序列用于保存参数的数组par[]清零，把索引
			 * 变量npar指向首项，并且设置我们开始处于取参数状态ESgetpars。但是如果此时接收到的字符c是'['，那么表明收到了键盘功能键发出的序列，于是设置下一状态为ESfunckey
			 * 并前去处理。如果接收到的字符不是'?'，则直接转到状态ESgetpars去处理。若接收到的字符是'?'，说明这个序列是终端设备私有序列，后面会有一个功能字符。于是去读下一字符
			 * 再到状态ESgetpars去处理代码处
			 */
			case ESsquare:
				...
				if ((ques=(c=='?')))
					break;
			/*
			 * 该状态表示我们此时要接收控制序列的参数值。参数用十进制数表示，我们把接收到的数字字符转换成数值并保存到par[]数组中，如果收到一个分号';'，则还是维持在本状态，并把
			 * 接收到的参数值保存在数组par[]下一项中。若不是数字字符或分号，说明已取得所有参数，那么就转移到状态ESgotpars去处理
			 */
			case ESgetpars:
				if (c==';' && npar<NPAR-1) {
					npar++;
			...
			/*
			 * ESgotpars状态表示我们已经接收到一个完整的控制序列。此时我们可以根据本状态接收到的结尾字符对相应控制序列进行处理。不过在处理之前，如果我们在SEsquare状态收到过'?'，
			 * 说明这个序列是终端设备私有序列。本内核不支持对这种序列的处理，于是我们直接恢复到ESnormal状态。否则就去执行相应控制序列。待序列处理完后就把状态恢复到ESnormal
			 */
			case ESgotpars:
				state = ESnormal;
				if (ques)								/* 收到了'?' */
				...
				switch(c) {
					 ...
					 * 如果c是'd'，则第1个参数带包光标所需在的行号（从0计数）。
					 * 如果c是'H'或'f'，则第1个参数代表光标移到的行号，第2个参数代表光标移动的列号
					 */
					...
					case 'u':							/* CSI u - 恢复保存的光标位置 */
						...
						break;
					/*
					 * 如果字符c是'l'或'b'，则分别表示设置屏幕黑屏间隔时间和设置粗体字符显示。此时参数数组中par[1]和par[2]是特征值，它们分别必须为par[1]=par[0]+13；
					 * par[2]=par[0]+17。在这个条件下，如果c是字符'l'，那么par[0]中是开始黑屏时说延迟的分钟数；如果c是字符'b'，那么par[0]中是设置的粗体字符属性值。
					 */
				...
				break;
			/* 状态ESfunckey表示我们接收到了键盘上功能键发出的一个序列。我们不用显示。于是恢复到正常状态ESnormal */
			case ESfunckey:
				state = ESnormal;
				break;
			...
			case ESsetterm:  /* Setterm functions. */
				...
				break;
			/* 
			 * 状态ESsetgraph表示收到设置字符集转义序列'ESC ('或'ESC )'。它们分别用于指定G0和G1所用的字符集。此时若收到字符'0'，则选择图形字符集作为G0和G1，若收到的字符是'B'， 
...
void con_init(void)
{
	...
	/* 首先根据setup.s程序取得的系统硬件参数初始化几个本函数专用的静态全局变量 */
	...
	blankcount = blankinterval;							/* 默认的黑屏间隔时间（滴答数） */
	
	/* 然后根据显示模式是单身还是彩色分别设置所使用的显示内存起始位置以及显示寄存器索引端口号和显示寄存器数据端口号。如果获得的BIOS显示方式等于7，则表示是单色显示卡 */
	if (ORIG_VIDEO_MODE == 7)	/* Is this a monochrome display? */
	{
		...
		/*
		 * 接着我们根据BIOS中断int 0x10功能0x12获得的显示模式信息，判断显示卡是单色显示卡还是彩色显示卡。若使用上述中断功能所得到的BX寄存器返回值不等于0x10，则说明
		 * 是EGA卡。因此初始显示类型为EGA单色。虽然EGA卡上有较多显示内存，但在单色方式下最多只能利用地址范围在0xb0000-0xb8000之间的显示内存。然后设置显示器描述字符
		 * 串为'EGAm'，并会在系统初始化期间显示器描述字符串将显示在屏幕的右上角。
		 * 注意，这里使用了BX寄存器在调用中断int 0x10前后是否被改变的方法来判断卡的类型。若BL在中断调用后值被改变，表示显示卡支持Ah=12h功能调用，是EGA或后推出来的VGA等
		 * 类型的显示卡。若中断调用返回值未变，表示显示卡不支持该功能，则说明是一般单色显示卡
		 */
	...
	else				/* If not, it is color. */
	{
		...
		/* 再判断显示卡类型。如果BX不等于0x10，则说明是EGA显示卡，此时共有32KB显示内存可用（0xb8000-0xc0000）。否则说明是CGA显示卡，只能使用8KB显示内存（0xb8000-0xba000） */
		if ((ORIG_VIDEO_EGA_BX & 0xff) != 0x10)
		{
	...
	/*
	 * 现在我们来计算当前显示卡内存上可以开设的虚拟控制台数量。硬件允许开设的虚拟控制台数量等于总显示内存量video_memery除以每个虚拟控制台占用的字节数。每个虚拟控制台占用的显示内存
	 * 数等于屏幕显示行数video_num_lines乘上每行字符占有的字节数video_size_row。如果硬件运行开设的虚拟控制台数量大于系统限定的最大数量MAX_CONSOLES，就把虚拟控制台数量设置为
	 * MAX_CONSOLES。若这样计算出的虚拟控制台数量为0，则设置为1（不可能吧！）。最后总显示内存数除以判断出的虚拟控制台数即得到每个虚拟控制台占用显示内存字节数
	 */
	...
	/* 
	 * 然后我们在屏幕的右上角显示描述字符串。采用的方法是直接将字符串写到显示内存的相应位置处。首先将显示指针display_ptr指到屏幕第1行右端差4个字符处（每个字符需2个字节，因此减8）
	 * 然后循环复制字符串的字符，并且每复制1个字符都空开1个属性字节
	 */
	...
	/* 
	 * 注意，此时当前虚拟控制台号currcons已被初始化为0.因此下面实际上是初始化0号虚拟控制台的结构vc_cons[0]中的所有字段值。例如，这里符号origin在前面已被定义为vc_cons[0].vc_origin
	 * 下面首先设置0号控制台的默认滚屏开始内存位置video_mem_start和默认滚屏末行内存位置，实际上它们也就是0号控制台占用的部分显示内存区域。然后初始设置0号虚拟控制台的其他属性和标志值
	 */
	...
	/* 
	 * 在设置了0号控制台当前光标所在位置和光标对应的内存位置pos后，我们循环设置其余的几个虚拟控制台结构的参数值。除了各自占用的显示内存开始和结束位置不同，它们的初始值基本上都与0号
	 * 控制台相同
	 */
	...
	/* 
	 * 最后设置当前前台控制台的屏幕原点（左上角）位置和显示控制器中光标显示位置，并设置键盘中断0x21陷阱门描述符（&keyboard_interrupt是键盘中断处理过程地址）。
	 * 然后取消中断控制芯片8259A中对键盘中断的屏蔽，允许响应键盘发出的IRQ1请求信号。最后复位键盘控制器以允许键盘开始正常工作
	 */
	update_screen();									/* 更新前台原点和设置光标位置 */
	set_trap_gate(0x21,&keyboard_interrupt);			/* 参加system.h */
...
/*
 * 打开蜂鸣
 * 8255A芯片PB端口的位1用作扬声器的开门信号；位0用作8253定时器2的门信号，该定时器的输出脉冲送往扬声器，作为扬声器发声的频率。因此要使扬声器蜂鸣，需要两步：首先
 * 开启PB端口（0x61）位1和位0（置位），然后设置定时器通道2发送一定的定时频率即可。参见boot/setup.s程序后8259A芯片编程方法和kernel/sched.c程序后的定时器编程说明
 */
static void sysbeep(void)
{
	/* enable counter 2 */	/* 开启定时器2 */
	outb_p(inb_p(0x61)|3, 0x61);
	/* set command for counter 2, 2 byte write */	/* 送设置定时器编程说明 */
	outb_p(0xB6, 0x43);									/* 定时器芯片控制字寄存器端口 */
	/* send 0x637 for 750 HZ */	/* 设置频率为750HZ，因此送定时值0637 */
...
int do_screendump(int arg)
{
	...
	/*
	 * 函数首先验证用户提供的缓冲区容量，若不够则进行适当扩展。然后从其开始处取出控制台号currcons。在判断控制台号有效之后，就把该控制台屏幕的所有内存内容复制
	 * 到用户缓冲区中
	 */
...
/*
 * 黑屏处理
 * 当用户在blankInterval时间间隔內没有按任何按键时就让屏幕黑屏，以保护屏幕
 */
void blank_screen()
{
	if (video_type != VIDEO_TYPE_EGAC && video_type != VIDEO_TYPE_EGAM)
...
/*
 * 控制台显示函数。
 * 该函数仅用于内核显示函数printk()（kernel/printk.c），用于在当前前台控制台上显示内核信息。处理方法是循环取出缓冲区中的字符，并根据字符的特性控制光标移动或直接显示在屏幕上
 * 参数b是null结尾的字符串缓冲区指针
 */
void console_print(const char * b)
{
	...
	/* 循环读取缓冲区b中的字符。如果当前字符c是换行符，则对光标执行回车换行操作；然后去处理下一个字符。如果是回车符，就直接执行回车动作。然后去处理下一个字符 */
	while ((c = *(b++))) {
		if (c == 10) {
			cr(currcons);
			lf(currcons);
		...
		/*
		 * 在读取了一个不是回车或换行字符后，如果发现当前光标列位置x已经到达屏幕右末端，则让光标折返到下一行开始处。然后把字符放到光标所处显示内存位置处，
		 * 即在屏幕上显示出来。再把光标右移一格位置，为显示下一个字符作准备
		 */
		if (x>=video_num_columns) {
			x -= video_num_columns;
			pos -= video_size_row;
