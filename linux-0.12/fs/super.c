...
int sys_umount(char * dev_name)
{
	...
	/*
	 * 首先根据设备文件名找到对应的i节点，并取其中的设备号。设备文件所定义设备的设备号是保存在其i节点的i_zone[0]中的
	 * 参见namei.c程序中系统调用sys_mknod()的代码。另外，由于文件系统需要存放在块设备上，因此如果不是块设备文件，则
	 * 放回刚申请的i节点dev_i，返回出错码
	 */
	...
	/*
	 * OK，现在上面为了得到设备号而取得的i节点已完成了它的使命，因此这里把它放回。接着我们来检查卸载该文件系统的条件是否满足。
	 * 如果是根文件系统，则不能被卸载，于是返回忙出错号。如果在超级块表中没有找到该设备上文件系统的超级块，或者该设备上文件系统没有
	 * 安装过，则返回出错码。如果超级块所指明的被安装到的i节点并没有置位其安装标志i_mount，则显示警告信息。然后查找i节点表，看看是否
	 * 有进程在使用该设备上的文件。如果有则返回忙出错码
	 */
	iput(inode);
	...
	/*
	 * 现在该设备上文件系统的卸载条件均得到满足，因此我们可以开始实施真正的卸载操作了。首先复位被安装到的i节点的安装标志，释放该i节点。然后
	 * 置超级块中被安装i节点字段为空，并放回设备文件系统的根i节点，接着置超级块中被安装系统根i节点指针为空
	 */
	/* 开始卸载操作 */
...
int sys_mount(char *dev_name, char *dir_name, int rw_flag)
{
	...
	/*
	 * 首先根据设备文件名找到对应的i节点，以取得其中的设备号。对于块特殊设备文件，设备号在其i节点的i_zone[0]中，
	 * 另外，由于文件系统必须在块设备中，因此如果不是块设备文件，则放回刚取得的i节点dev_i，返回出错码
	 */
	/* 检查设备名是否有效 */
	...
	/*
	 * OK，现在上面为了得到设备号而取得的i节点dev_i已完成了它的使命，因此这里放回设备文件的i节点。接着我们来检查一下
	 * 文件系统安装到的目录名是否有效。于是根据给定的目录文件名找到对应的i节点dir_i。如果该i节点的引用计数不为1（仅在这里引用），
	 * 或者该i节点的节点号是根文件系统的节点号1，则放回该i节点返回出错码。另外，如果该节点不是一个目录文件节点，则也放回该i节点，
	 * 返回出错码。因为新文件系统只能安装在一个目录名上
	 */
	iput(dev_i);
	...
	/*
	 * 现在安装点也检查完毕，我们开始读取要安装文件系统（新文件系统）的超级块信息。一个文件系统的超级块
	 * 会首先从超级块表中进行搜索，如果不在超级块表中就从设备上读取。在得到了文件系统超级块之后，我们对
	 * 它先进行检查一番。如果文件系统已安装在其他地方，或者将要安装到的i节点已经安装了文件系统（安装标志已经置位）
	 * 则都放回该i节点，也返回出错码。
	 */
	/* 读取要安装文件系统的超级块信息 */
...
void mount_root(void)
{
	...
	/*
	 * 首先检查一下磁盘i节点大小是否符合要求（32字节），以防止修改代码时出现不一致情况。然后初始化
	 * 文件表数组（共64项，即系统同时只能打开64个文件）和超级块表（8项）。这里将所有文件结构中的引用
	 * 计数设置为0（表示空闲），并把超级块表中各项结构的设备字段初始化为0（也表示空闲）。如果根文件
	 * 系统所在设备是软盘的话，就提示“插入根文件系统盘并按回车键”，并等待按键
	 */
	...
	/* 
	 * 现在我们对超级块和根i节点进行设置。把根i节点引用次数递增3次。因为随后几行代码上也引用了该i节点，
	 * 并且iget()函数中i节点引用次数已被设置为1。然后设置该超级块的被安装文件系统根目录i节点字段（s_isup）和
	 * 被安装到的i节点字段（s_imount）为该i节点，再设置当前进程的当前工作目录和根目录i节点。此时
	 * 当前进程是1号进程（init进程）
	 */
	...
	current->root = mi;
	/*
	 * 然后我们对根文件系统上的资源作统计工作，统计该设备上空闲块数和空闲i节点数。首先令i等于超级块中表明的设备逻辑块总数。
	 * 然后根据逻辑块位图中相应比特位的占用情况统计出空闲块数。这里宏函数set_bit()只测试比特位。“i&8191”用于取得i节点
	 * 号在当前位图中对应的比特偏移值。“i>>13”是讲i除以8192，也即除一个磁盘块包含的比特位数。
	 */
	/* 对根文件系统上的资源作统计工作 */
	/* 统计该设备上空闲块数 */
	free = 0;
	...
	/*
	 * 在显示过设备上空闲逻辑块数/逻辑块总数之后，我们再统计设备上空闲i节点数。首先令i等于超级块中表明的设备i节点总数+1
	 * （加1是将0节点也统计进去），然后根据i节点位图中相应比特位的占用情况计算出空闲i节点数。最后再显示设备上可用空闲i节点数和i节点总数
	 */
